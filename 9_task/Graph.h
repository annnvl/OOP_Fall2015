...
/*
std::ostream& operator<<(std::ostream& os, std::pair<bool, int>& pr){
...
public:
    ...
    void print(){
        for (auto y:_vertexes) {
            std::cout << _data[y.first].end << " " << _data[y.first].sufflink << " " << _data[y.first].reslen << " " << _data[y.first].suffout << "[" << y.first << "]" << " -> ";
            for (auto z=y.second.begin(); z != y.second.end(); ++z) {
                std::cout << z->first << "(" <<(char)z->second << ") ";
            }
            std::cout << "\n";
        }
    };
